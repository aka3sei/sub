...
# --- メイン評価エリア（B/C/D 項目省略なし） ---
col1, col2, col3 = st.columns(3)

# 【B】数値評価
with col1:
    st.header("【B】数値評価 (60%)")
    s_target = st.number_input("売上目標", value=1000, key="st")
    s_actual = st.number_input("売上実績", value=900, key="sa")
    s_rate = (s_actual / s_target) if s_target > 0 else 0
    p_target = st.number_input("粗利目標", value=300, key="pt")
    p_actual = st.number_input("粗利実績", value=310, key="pa")
    p_rate = (p_actual / p_target) if p_target > 0 else 0
    n_target = st.number_input("新規目標", value=10, key="nt")
    n_actual = st.number_input("新規実績", value=8, key="na")
...
with res_col2:
    ...
    # 差分の計算（調整によってどれだけ増減したか）
    diff_rate = total_rate - final_rate
    
    r1, r2 = st.columns(2)
    r1.metric("最終支給額", f"¥{final_amount:,}")
    
    # 【ここを修正】delta_colorを "normal" に戻し、差分を表示
    # 係数が1.0より大きければ青（緑）、小さければ赤、1.0ならグレーになります
    st.metric(
        label="最終支給率（調整後）", 
        value=f"{total_rate:.2%}", 
        delta=f"調整前 {final_rate:.2%}",
        delta_color="normal" # normalにするとプラスなら緑、マイナスなら赤
    )
